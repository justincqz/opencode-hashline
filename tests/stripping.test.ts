...
describe('detectLineEnding', () => {
    ...
    test('single line no newline returns lf', () => {
        const input = 'single line no newline';
        expect(detectLineEnding(input)).toBe('lf');
    });

    test('empty string returns lf', () => {
        const input = '';
        expect(detectLineEnding(input)).toBe('lf');
    });

    test('single character returns lf', () => {
        const input = 'a';
        expect(detectLineEnding(input)).toBe('lf');
    });
});
...
describe('reconstructReadOutput', () => {
    ...
    test('reconstructs correctly with pipe format', () => {
        const parsed: any = {
            beforeContent: '<path>/test.ts</path>\n<content>\n',
            contentLines: [{ lineNum: 1, content: 'const x = 1' }],
            afterContent: '\n</content>',
            format: 'pipe',
        };
